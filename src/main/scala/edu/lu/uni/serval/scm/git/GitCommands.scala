...
object GitCommands
{
  ...
  def archiveCommit(gitBase: String, outputDir: String, hash: String): Boolean =
  {
    ...
    if(exitCode != 0)
    {
      Console.err.println("git archiving for %s has failed.".format(hash));
      return false;
    }

    println("git archiving: %s".format(hash));
    ...
    if(exitCode != 0)
    {
      Console.err.println("Unable to unzip %s.".format(hash))
      return false;
    }

    println("unzip: %s".format(hash))
  ...
  def clone(url: String, path: String, workingDir: String): Int =
  {
    ...
    println("cloning result: " + success);
